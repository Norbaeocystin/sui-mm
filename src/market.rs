...
pub async fn get_fills(client: &SuiClient){
    ...
    //     let price = item.parsed_json.get("price").unwrap();
    //     let base_asset_quantity_filled = item.parsed_json.get("base_asset_quantity_filled").unwrap();
    //     let base_asset_quantity_remaining = item.parsed_json.get("base_asset_quantity_remaining").unwrap();
...
pub fn calculate(events: &Vec<SuiEvent>){
